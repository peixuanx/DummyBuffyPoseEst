function cost =  DummyPose(img,det_bb)
...
%          torso, left upper arm, right upper arm, left lower arm, right lower arm, head
  ...
  match_energy_cost(L,part,seq);
  normalizded_det_size = [90 100];
  x1 = rand(1,4)*normalizded_det_size(2);
  x2 = rand(1,4)*normalizded_det_size(2);
  y1 = rand(1,4)*normalizded_det_size(1);
  y2 = rand(1,4)*normalizded_det_size(1);
